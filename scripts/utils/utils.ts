...
export function displayGasCostsMatrix(gasCosts, networks) {
    const header = ['networks  ', 'gasPrice Gwei', 'deploy $  ', 'registerDataProvider $  ', 'updatePrice $  ', 'avg. per updatePrice call ', 'total # of txs  ', 'total cost  '];
    const networkData = [];

    for (let network of networks) {
        const gasPrice = ethers.utils.formatUnits(gasCosts[network.name].update.gasPrice, "gwei");
        const deploy = ethers.BigNumber.from(gasCosts[network.name].deploy.toString());
        
        const deployCost = ethers.utils.formatEther(deploy.toString());
        const registerCost = ethers.utils.formatEther(gasCosts[network.name].register.toString());
        ...
        for (const dataProvider in updateIndividualProviders) {
            ...
            avgCostPerUpdate = avgCostPerUpdate.add(ethers.BigNumber.from(updateIndividualProviders[dataProvider].totalGas.toString()));
        }
        avgCostPerUpdate = avgCostPerUpdate.div(totalTxs);
        
        const totalCost = ethers.utils.formatEther(
            deploy.add(ethers.BigNumber.from(gasCosts[network.name].register.toString()))
                .add(ethers.BigNumber.from(gasCosts[network.name].update.totalGasCost.toString()))
        );
    ...
    const columnWidths = header.map((_, index) => {
        return Math.max(...networkData.map(row => row[index].toString().length), header[index].length) + 1;
    });
    ...
    console.log("\n- # OF DATA PROVIDERS:", 3);
    console.log("\n");
    console.log('-'.repeat(columnWidths.reduce((acc, width) => acc + width, 0) + (columnWidths.length - 1)));
