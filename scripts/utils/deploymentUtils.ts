 ...
 */
export async function deployZkSyncLocalTestnet(networkConfig: any, deployer: ethers.Wallet): Promise<DeployedContract> {
...
export async function deployOptimismTestnet(networkConfig: any, deployer: ethers.Wallet): Promise<DeployedContract> {
...
export async function deployContract(networkName: string, deployerWallet: any): Promise<{ address: string, gas: number }> {
    ...
    let deployer = networkName === LOCALHOST_URL
    ...
    const gasPrice = receipt.deployTransaction.gasPrice || ethers.BigNumber.from(250_000_000);
