...
import { registerDataProviders, updatePrices } from "./oracleOps";
...
const dataProviders = Array(3).fill(null).map(() => new ethers.Wallet(ethers.Wallet.createRandom().privateKey));
...
async function main() {
    for (let networkConfig of networks) {
        ...
        const deployer = new ethers.Wallet(networkConfig.richWalletPK).connect(provider);
        ...
        gasCosts[networkConfig.name].register = await registerDataProviders(contract.address, networkConfig.rpcEndpoint, connectedWallets);

        // Update prices
        const balances = await Promise.all(connectedWallets.map(wallet => wallet.getBalance()));
        console.log("\nBalance of dataProviders before updatePrice: ", balances.map(ethers.utils.formatEther));
        console.log("Updating prices: ");
        gasCosts[networkConfig.name].update = await updatePrices(contract.address, networkConfig.rpcEndpoint, connectedWallets);
